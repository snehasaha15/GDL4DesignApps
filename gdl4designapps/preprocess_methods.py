...
class losses:
    ...
    def classifier_losses(S_in, S_out, pcs_t, ind_t, class_prob_s, c_label, class_label_c, alpha, beta):
        ''' Mean squared distance (MSD) measured for a batch of organized
        3D point clouds and weighted by the selection probability of the
        templates (Split-AE).
        ...
        loss_classifier_content = tf.reduce_mean(
            tf.nn.softmax_cross_entropy_with_logits(labels=c_label, logits=class_label_c),
            name='loss_ce_content')
        style_loss = tf.math.scalar_mul(alpha, loss_classifier_style)
        content_loss = tf.math.scalar_mul(beta, loss_classifier_content)
...
class arch_training:
    ...
    def splitaetemp_loader(config, pc_size, normlimits):
        ''' Load the templates of the style classes
        ...
        #one hot encoding of input shape label
        #[1,0] for cars and [0,1]- airplane
        c_label = list(config["content_label"])
    ...
    def splitae_training(splitae_config, GPUid=-1):
        ...
        if os.path.exists(splitae_config):
            os.system("cp {} configdict_splitae.py".format(splitae_config))
            from configdict import confignet as config
        else:
            print("ERROR! Configuration file not found!")
        ...
        ## Load templates
        pcs_t, ind_t, c_label = arch_training.splitaetemp_loader(config, pc_size,
                                                              normlimits)
        ...
        decoder_sizes = list(config["decoder_layers"])
        decoder_sizes.append(pc_size)
        # content and style classifier layers
        mlp_layers_content = config["mlp_content_layers"]
        mlp_layers_style = config["mlp_style_layers"]
        ...
        # Optimizer and losses
        optimizer = tf.train.AdamOptimizer(l_rate)
        rec_loss =  losses.msd_gpu(S_in, S_out)
        loss_comb = losses.classifier_losses(S_in, S_out, pcs_t, ind_t, class_prob_s, c_label, class_label_c, alpha, beta)
